...
if [ "$SHELL" == "/bin/bash" ]
then 
echo "This is a bash shell - exporting value to the LD_LIBRARY_PATH for the initial build"
export LD_LIBRARY_PATH=$LD_LIB_dir:$LD_LIBRARY_PATH
fi

if [ "$SHELL" == "/bin/csh" ]
then 
echo "This is a csh - exporting value to the LD_LIBRARY_PATH for the initial build"
