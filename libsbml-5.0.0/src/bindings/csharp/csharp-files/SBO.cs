 ...
 * parameter with identifier <c>k</c> that would indicate to a
 * software tool it is a first-order rate constant (if that's what
 * <c>k</c> happened to be in some given model).  However, one may
...
public class SBO : IDisposable {
 ...
 public static string intToString(int sboTerm) {
   ...
   * in the correct format (a zero-padded, seven digit string), <c>-1</c> is
   * returned.
   */
